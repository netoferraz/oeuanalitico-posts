        ...
        for desc in dadoNfe.iterdescendants():
            if desc.text_content().strip() == 'Data de Emissão':
                ...
                if not 'br' in dadoNota.tag:
                    inputDataNfe.modelo = dadoNota.text_content().strip()
                else:
                    inputDataNfe.modelo = dadoNota.getnext().text_content().strip()
            elif desc.text_content().strip() == 'Série':
                serieNfe = desc.getnext()
                if not 'br' in serieNfe.tag:
                    inputDataNfe.serie = serieNfe.text_content().strip()
                else:
                    inputDataNfe.serie = serieNfe.getnext().text_content().strip()
                ...
                if not 'br' in numeroNfe.tag:
                    inputDataNfe.valor = valorNfe.text_content().strip()
                else:
                    try:
                        inputDataNfe.valor = valorNfe.getnext().text_content().strip()
                    ...
                    if 'span' in desc.tag:
                        ...
                        if "TextoFundoBrancoNegrito" in desc.items()[0][1]:
                            if desc.text == "Descrição":
                                value = desc.getnext()
                                if "br" in value.tag:
                                    nome = value.getnext().text_content().strip()
                                    prod.nome = nome
                ...
                for desc in morefeature.iterdescendants():
                    if 'span' in desc.tag:
                        if desc.text_content().strip() == 'Código do produto':
                            ...
                            prod.valor_unit_cmc = valor_unit
                            ...
                            prod.valor_unit_trib = valor_un_trib
